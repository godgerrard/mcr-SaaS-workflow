...
## Auth / access
- **Final service-key rotation at pilot handover** — every rotation so far has leaked into a Claude session transcript one way or another (direct paste, unfiltered file read, or the harness auto-injecting the .env.local diff when it was edited mid-session). Before the pilot org gets real data: rotate once more in Supabase → Settings → API and update `mcr/.env.local` while NO Claude session is running. Key is deployed nowhere, so nothing else changes.
